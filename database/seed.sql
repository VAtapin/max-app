...
'[ИСПОЛНИТЕЛЬ] предлагает предоставить платный доступ к кабинету лидера SWPro. Стоимость доступа рассчитывается по лимиту активных консультантов, доступных лидеру в выбранном периоде.

Стоимость: [СТОИМОСТЬ] рублей в месяц за согласованный лимит консультантов. Оплата на первом этапе подтверждается администратором вручную. Срок доступа указывается в кабинете лидера.
...
('Подключения VK и OK', 'Раздел Подключения нужен для отправки ответов и рассылок через сообщества VK и группы OK. Это не личная страница консультанта: сообщения уходят от имени сообщества или группы.', JSON_ARRAY(
  ...
  'ID группы/канала: числовой ID сообщества VK или группы OK.',
...
('Как подключить VK-сообщество', 'Для VK нужен ключ доступа сообщества с правом отправки сообщений. Пользователь должен открыть диалог с сообществом или разрешить сообщения, иначе VK не даст отправить ему личное сообщение.', JSON_ARRAY(
  ...
  'Скопируйте числовой ID сообщества: для vk.com/club12345 это 12345, для vk.com/public12345 это 12345.',
  'В SWPro откройте Подключения, выберите VK, укажите название, ID сообщества и ключ доступа, затем сохраните.'
), 0, 110),
