...
В приложении OK на странице «Связаться» клиент видит блок **Личные сообщения OK**. Он нажимает **Разрешить сообщения OK** и подтверждает системное окно Одноклассников. В текущем VK Mini App это окно открывается нативным методом VK Bridge, который поддерживается платформой OK. После этого консультант может писать клиенту через живой чат, «Что сделать сегодня» и AI-студию.

Если клиент открыл обычный Web-мини-сайт, системное окно недоступно. Вместо ссылки на группу будет кнопка **Открыть Mini App в OK**: она открывает настоящее приложение SWPro в Одноклассниках, где клиент подтверждает разрешение штатной кнопкой. Вступать в группу для этого не требуется.
