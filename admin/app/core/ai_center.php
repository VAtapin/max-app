...
function ai_openai_studio_draft(string $type, string $subject, string $facts, array $personalization = []): array
{
    ...
    $formats = [
        ...
        'voice_script' => 'Сценарий личного голосового сообщения длительностью до 35 секунд. Пиши так, как живой человек говорит одному знакомому: короткими разговорными фразами, тепло и без канцелярита. Только произносимый текст — без заголовков, списков, Markdown, эмодзи, ссылок, служебных пометок и режиссёрских комментариев. Не используй рекламную или торжественную подачу. Обращение по имени должно звучать естественно, а не как официальное объявление.',
    ...
    $isPublicMaterial = in_array($type, ['post', 'product_description'], true);
    ...
    $personalBlock = !$isPublicMaterial && ($profileLines || $checkup !== '')
        ? implode("\n", $profileLines) . "\nОбезличенный контекст последнего чек-апа:\n" . ($checkup !== '' ? $checkup : 'нет')
        : 'Персональный получатель не выбран.';

    $personalizationRules = $isPublicMaterial
        ? 'Это публичный материал. Не упоминай выбранного клиента и не используй его имя, возраст, дату рождения, пол, город, результаты или сам факт прохождения чек-апа.'
        : implode("\n", [
            'Сведения профиля — внутренний контекст для выбора тона и содержания, а не текст для публикации.',
            'Можно естественно обратиться только по имени и не более одного раза. Не используй фамилию без прямой необходимости.',
            'Никогда не перечисляй и не заключай в скобки возраст, дату рождения, пол, город или другие поля анкеты.',
            'Не пиши формулировки вроде «Мария (Москва, 50 лет)», «женщина 50 лет», «в вашем последнем чек-апе отмечено» и не показывай баллы, названия шкал или служебные категории.',
            'Возраст, город, пол и контекст чек-апа могут только незаметно помочь выбрать уместный тон или тему. Упоминай конкретный факт лишь когда пользователь прямо запросил его в теме и он необходим для результата.',
        ]);
    ...
    return ai_openai_response([
        ...
        'instructions' => implode("\n", [
            ...
            'Контекст чек-апа используй только для уместной персонализации; не называй его диагнозом, не делай медицинских выводов и не раскрывай исходные ответы.',
