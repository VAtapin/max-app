...
function crud_display_columns(string $moduleKey): array
{
    return [
        ...
        'integrations' => [
            ...
            'callback_subscribed_at' => 'OK webhook',
    ...
    if ($moduleKey === 'resellers') {
        ...
        return [
            "SELECT r.id, r.parent_reseller_id, parent.name AS parent_name,
                    r.name, r.email, r.phone, r.legal_name, r.legal_status, r.legal_inn, r.legal_address,
        ...
        return [
            "SELECT i.id, i.title,
                    CASE
                        ...
                        ELSE 'Общее'
                    END AS owner_label,
                    i.platform, i.external_id, i.is_default, i.callback_subscribed_at, i.callback_last_event_at, IF(i.is_active = 1, 'active', 'inactive') AS state
...
function crud_cell_value(string $moduleKey, string $column, array $row): string
{
    ...
    if ($moduleKey === 'integrations' && in_array($column, ['callback_subscribed_at', 'callback_last_event_at'], true)) {
...
{
    return match ($moduleKey) {
        'resellers' => ['id', 'name', 'legal_name', 'parent_name', 'email', 'phone', 'legal_inn', 'referral_code', 'subscription_plan_title', 'subscription_status', 'state'],
...
function crud_paginated_rows(string $moduleKey, string $sql, array $params, array $columns): array
{
    $searchColumns = array_intersect(crud_search_columns($moduleKey), array_merge(array_keys($columns), [
        ...
        'platform_accounts_summary', 'legal_name', 'legal_inn',
    ]));
