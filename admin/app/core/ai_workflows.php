...
function ai_workflow_ensure_plan(array $user, int $sessionId): ?int
{
    ...
    try {
        ...
        $productStmt = db()->prepare('SELECT p.id source_id, "product" source_type, p.title, COALESCE(NULLIF(r.reason_text, ""), p.usage_text, p.short_description) advice_text FROM recommendations r INNER JOIN products p ON p.id = r.product_id AND p.is_active = 1 AND p.is_deleted = 0 AND p.ai_enabled = 1 AND p.content_status = "approved" AND (p.product_kind NOT IN ("supplement","food") OR (p.safety_review_status = "verified" AND NULLIF(p.composition, "") IS NOT NULL AND NULLIF(p.usage_text, "") IS NOT NULL AND NULLIF(p.warning_text, "") IS NOT NULL AND NULLIF(p.contraindications, "") IS NOT NULL AND NULLIF(p.allowed_claims, "") IS NOT NULL AND NULLIF(p.source_urls, "") IS NOT NULL)) WHERE r.test_session_id = :session_id ORDER BY r.score DESC, r.id DESC LIMIT 3');
