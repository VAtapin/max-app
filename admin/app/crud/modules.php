...
return [
    ...
    'products' => [
        ...
        'table' => 'products',
        'columns' => ['id', 'category_id', 'title', 'catalog_sku', 'product_kind', 'price', 'image_review_status', 'is_active'],
        'fields' => [
            'category_id' => ['label' => app_text('auto.k_1cf49d95b0ed'), 'type' => 'select', 'source' => 'product_categories', 'nullable' => true],
            'title' => ['label' => app_text('auto.k_3de49828e86a'), 'required' => true],
            'slug' => ['label' => 'Slug', 'required' => true],
            'catalog_sku' => ['label' => 'Основной артикул'],
            'catalog_source' => ['label' => 'Источник каталога'],
            'catalog_page' => ['label' => 'Страница каталога', 'type' => 'number', 'nullable' => true],
            'product_kind' => ['label' => 'Тип продукта', 'type' => 'select', 'options' => ['supplement', 'food', 'cosmetic', 'fragrance', 'personal_care', 'other'], 'required' => true],
            'safety_review_status' => ['label' => 'Проверка безопасности', 'type' => 'select', 'options' => ['not_required', 'catalog_only', 'verified'], 'required' => true],
            'image_review_status' => ['label' => 'Проверка изображения', 'type' => 'select', 'options' => ['missing', 'candidate', 'approved', 'rejected'], 'required' => true],
            'recommendation_notice' => ['label' => 'Предупреждение в рекомендации', 'type' => 'textarea'],
    ...
    'product_variants' => [
        'title' => 'Варианты и артикулы продуктов',
        'table' => 'product_variants',
        'columns' => ['id', 'product_id', 'sku', 'title', 'volume_text', 'price', 'is_sample', 'is_active'],
        'fields' => [
            'product_id' => ['label' => 'Продукт', 'type' => 'select', 'source' => 'products', 'required' => true],
            'sku' => ['label' => 'Артикул', 'required' => true],
            'title' => ['label' => 'Название варианта'],
            'volume_text' => ['label' => 'Объём или упаковка'],
            'price' => ['label' => 'Справочная цена', 'type' => 'number', 'step' => '0.01', 'nullable' => true],
            'currency' => ['label' => 'Валюта', 'default' => 'RUB'],
            'image_path' => ['label' => 'Изображение продукта', 'type' => 'file', 'accept' => 'image/*'],
            'is_sample' => ['label' => 'Пробник', 'type' => 'checkbox', 'default' => 0],
            'sort_order' => ['label' => app_text('auto.k_ed030118aad8'), 'type' => 'number', 'default' => 100],
            'is_active' => ['label' => app_text('auto.k_667904ef22a4'), 'type' => 'checkbox', 'default' => 1],
        ],
    ],
    'recommendation_signals' => [
        'title' => 'Сигналы рекомендаций',
        'table' => 'recommendation_signals',
        'columns' => ['id', 'title', 'slug', 'safety_level', 'is_active', 'sort_order'],
        'fields' => [
            'title' => ['label' => 'Потребность клиента', 'required' => true],
            'slug' => ['label' => 'Slug', 'required' => true],
            'description' => ['label' => 'Описание', 'type' => 'textarea'],
            'keywords_json' => ['label' => 'Ключевые слова (JSON)', 'type' => 'textarea'],
            'safety_level' => ['label' => 'Уровень осторожности', 'type' => 'select', 'options' => ['general', 'caution', 'specialist'], 'required' => true],
            'sort_order' => ['label' => app_text('auto.k_ed030118aad8'), 'type' => 'number', 'default' => 100],
            'is_active' => ['label' => app_text('auto.k_667904ef22a4'), 'type' => 'checkbox', 'default' => 1],
        ],
    ],
    'product_signal_links' => [
        'title' => 'Связи продуктов и потребностей',
        'table' => 'product_signal_links',
        'columns' => ['id', 'product_id', 'signal_id', 'match_type', 'weight', 'is_approved'],
        'fields' => [
            'product_id' => ['label' => 'Продукт', 'type' => 'select', 'source' => 'products', 'required' => true],
            'signal_id' => ['label' => 'Потребность клиента', 'type' => 'select', 'source' => 'recommendation_signals', 'required' => true],
            'match_type' => ['label' => 'Действие', 'type' => 'select', 'options' => ['supports', 'exclude'], 'required' => true],
            'weight' => ['label' => 'Вес рекомендации', 'type' => 'number', 'default' => 100, 'required' => true],
            'rationale' => ['label' => 'Почему подходит или исключается', 'type' => 'textarea'],
            'is_approved' => ['label' => 'Связь проверена', 'type' => 'checkbox', 'default' => 0],
        ],
    ],
    'product_media' => [
        'title' => 'Изображения продуктов',
        'table' => 'product_media',
        'columns' => ['id', 'product_id', 'variant_id', 'media_type', 'source_page', 'review_status', 'is_primary'],
        'fields' => [
            'product_id' => ['label' => 'Продукт', 'type' => 'select', 'source' => 'products', 'required' => true],
            'variant_id' => ['label' => 'Артикул', 'type' => 'select', 'source' => 'product_variants', 'nullable' => true],
            'media_type' => ['label' => 'Тип', 'type' => 'select', 'options' => ['product_packshot', 'document', 'video'], 'required' => true],
            'file_path' => ['label' => 'Файл', 'type' => 'file', 'accept' => 'image/*,application/pdf,video/*', 'required' => true],
            'source_page' => ['label' => 'Страница каталога', 'type' => 'number', 'nullable' => true],
            'review_status' => ['label' => 'Статус проверки', 'type' => 'select', 'options' => ['candidate', 'approved', 'rejected'], 'required' => true],
            'is_primary' => ['label' => 'Основное изображение', 'type' => 'checkbox', 'default' => 0],
        ],
    ],
    'tests' => [
