...
<?php if (!empty($draft['end_user_id'])): ?><div class="form-actions ai-draft-send-actions"><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>"><input type="hidden" name="action" value="send_draft"><input type="hidden" name="id" value="<?= (int)$draft['id'] ?>"><label class="field"><span>Канал</span><select name="channel"><option value="">Автоматически</option><option value="telegram">Telegram</option><option value="VK">VK</option><option value="OK">OK</option><option value="MAX">MAX</option><option value="web">Web-чат</option></select></label><button type="submit">Отправить клиенту</button></form><a class="button secondary-button" href="index.php?chat_user_id=<?= (int)$draft['end_user_id'] ?>#live-chat">Открыть чат</a><small class="field-hint">Выберите OK, если клиент разрешил сообщения этой группы. Перед отправкой сохраните исправленный текст.</small></div><?php endif; ?>
