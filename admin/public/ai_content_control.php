    ...
    try {
        if ($action === 'sync_docs') {
            ...
            if ($action === 'save_product') {
                ...
                $safetyStatus = in_array($_POST['safety_review_status'] ?? '', ['not_required', 'catalog_only', 'verified'], true) ? $_POST['safety_review_status'] : 'catalog_only';
                $imageStatus = in_array($_POST['image_review_status'] ?? '', ['missing', 'candidate', 'approved', 'rejected'], true) ? $_POST['image_review_status'] : 'missing';
                if (!$current) {
                ...
                $stmt = db()->prepare('UPDATE products SET allowed_claims = :allowed_claims, source_urls = :source_urls, safety_review_status = :safety_review_status, image_review_status = :image_review_status, ai_enabled = :ai_enabled, content_status = :content_status, reviewed_by = :reviewed_by, reviewed_at = IF(:content_status_check = "approved", NOW(), NULL), next_review_at = :next_review_at WHERE id = :id');
                $stmt->execute($common + ['allowed_claims' => $allowedClaims ?: null, 'safety_review_status' => $safetyStatus, 'image_review_status' => $imageStatus, 'content_status_check' => $status]);
            } else {
...
$products = db()->query('SELECT id, title, catalog_sku, catalog_page, product_kind, safety_review_status, image_review_status, composition, usage_text, warning_text, contraindications, allowed_claims, source_urls, ai_enabled, content_status, next_review_at FROM products WHERE is_deleted = 0 ORDER BY content_status, title LIMIT 1000')->fetchAll();
...
<?php foreach ($products as $row): ?><details class="faq-manage-item"><summary><strong><?= h((string)$row['title']) ?></strong><span class="cell-muted"><?= $row['catalog_sku'] ? 'арт. '.h((string)$row['catalog_sku']).' · ' : '' ?><?= h(ai_control_status_label((string)$row['content_status'])) ?> · <?= (int)$row['ai_enabled'] ? 'доступен ИИ' : 'не используется ИИ' ?></span></summary><form method="post" class="crud-form"><input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>"><input type="hidden" name="action" value="save_product"><input type="hidden" name="product_id" value="<?= (int)$row['id'] ?>"><label class="field wide"><span>Допустимые обещания и формулировки</span><textarea name="allowed_claims" rows="3"><?= h((string)$row['allowed_claims']) ?></textarea></label><label class="field wide"><span>Первоисточники и утверждённые материалы</span><textarea name="source_urls" rows="3"><?= h((string)$row['source_urls']) ?></textarea></label><label class="field"><span>Проверка безопасности</span><select name="safety_review_status"><?php foreach (['not_required'=>'Не требуется','catalog_only'=>'Только каталог','verified'=>'Инструкция проверена'] as $key=>$label): ?><option value="<?= $key ?>" <?= $row['safety_review_status']===$key?'selected':'' ?>><?= $label ?></option><?php endforeach; ?></select></label><label class="field"><span>Проверка изображения</span><select name="image_review_status"><?php foreach (['missing'=>'Нет изображения','candidate'=>'Кандидат','approved'=>'Проверено','rejected'=>'Отклонено'] as $key=>$label): ?><option value="<?= $key ?>" <?= $row['image_review_status']===$key?'selected':'' ?>><?= $label ?></option><?php endforeach; ?></select></label><label class="field"><span>Статус</span><select name="content_status"><?php foreach (['draft'=>'Черновик','review'=>'На проверке','approved'=>'Утверждено'] as $key=>$label): ?><option value="<?= $key ?>" <?= $row['content_status']===$key?'selected':'' ?>><?= $label ?></option><?php endforeach; ?></select></label><label class="field"><span>Следующая проверка</span><input type="date" name="next_review_at" value="<?= h((string)$row['next_review_at']) ?>"></label><label class="check-row"><input type="checkbox" name="ai_enabled" value="1" <?= (int)$row['ai_enabled'] ? 'checked' : '' ?>><span>Разрешить использовать в ответах ИИ</span></label><div class="form-actions"><a class="button secondary-button" href="crud.php?module=products&action=edit&id=<?= (int)$row['id'] ?>">Открыть продукт</a><button type="submit">Сохранить контроль</button></div></form></details><?php endforeach; ?></section>
