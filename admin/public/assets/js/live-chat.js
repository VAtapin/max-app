(() => {
    ...
    const contactBadges = (item) => {
        ...
        if (String(item.phone || '').trim()) labels.push('Телефон');
        return [...new Set(labels)].map(label => `<i>${escapeHtml(label)}</i>`).join('');
    };
